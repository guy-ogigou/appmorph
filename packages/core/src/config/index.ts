...
export function loadConfig(): AppConfig {
    ...
    if (existsSync(defaultPath)) {
      ...
      if (existsSync(altPath)) {
        ...
        errors.push({
          ...
          message:
            "Project path not specified and default path not found. Set APPMORPH_PROJECT_PATH environment variable.",
        });
      }
    }
  } else {
  ...
  // Sanitizer configuration (optional - only enabled if OPENAI_API_KEY is set)
  const openaiApiKey = process.env.OPENAI_API_KEY;
  const sanitizerConfig = openaiApiKey
    ? {
        openaiApiKey,
        openaiModel: process.env.OPENAI_MODEL || "gpt-4o-mini",
        intervalMs: parseInt(process.env.SANITIZER_INTERVAL_MS || "2000", 10),
      }
    : undefined;
